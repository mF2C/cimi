(ns com.sixsq.slipstream.ssclj.resources.qos-model-lifecycle-test
  (:require
    ...
    [com.sixsq.slipstream.ssclj.resources.qos-model :as qos-model]
    [com.sixsq.slipstream.ssclj.resources.qos-model :as callback]
    [peridot.core :refer :all]))

(use-fixtures :each ltu/with-test-server-fixture)

(def base-uri (str p/service-context (u/de-camelcase qos-model/resource-url)))
...
(deftest lifecycle
  (let [session       (-> (ltu/ring-app)
    ...
    (let [create-test-callback {:id                    (str callback/resource-url "/example-qos-model")
                                :resourceURI           callback/resource-uri
                                :acl                   valid-acl
                                :service               {:href "service/service-id"}
                                :agreement             {:href "agreement/agreement-id"}
                                :agents                [{:href "agent/agent-id-1"}
                                                        {:href "agent/agent-id-2"}]
                                :config                "test"
